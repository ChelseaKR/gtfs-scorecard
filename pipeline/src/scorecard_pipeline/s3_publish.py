...
def publish_tree(
    ...
    *,
    ...
    Additive by design: nothing is ever deleted, matching the ``aws s3 sync``
    call this replaces. S3 stays the durable record of dated artifacts that a
    fresh checkout does not carry, and pruning stays a bucket lifecycle
    concern.
    """
    if not root.is_dir():
