...
`routes.txt` lets you set `route_color` (the route's background color) and
`route_text_color` (the color of the route name drawn on it). The two are too
...
- **Set `route_text_color` to whichever of black (`000000`) or white
  (`FFFFFF`) contrasts more with the route color.** Dark route colors take
  white text; light ones take black.
- Aim for a contrast ratio of at least 4.5:1 (the WCAG AA threshold). Any online
  contrast checker will tell you whether a color pair passes.
- If a route has no strong brand color, leaving both fields blank is fine; apps
  fall back to readable defaults.

## How long it usually takes

A few minutes: pick black or white text per route color. A bulk edit in most
scheduling tools.
