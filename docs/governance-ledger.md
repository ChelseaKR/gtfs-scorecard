...
| 7 perf | applicable (lab budgets) | Lighthouse budgets gate the Pages deploy: performance ≥ 0.90, LCP ≤ 2500 ms, CLS ≤ 0.1, TBT ≤ 200 ms (`lighthouserc.json`). Load testing is N/A: the product is a static site with no server latency contract; the only hosted compute (`infra/`) is unapplied. |
