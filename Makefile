...
verify:
	...
	cd pipeline && uv run ruff check src tests scripts/generate_iso3166.py scripts/check_site_seo.py
	cd pipeline && uv run ruff format --check src tests scripts/generate_iso3166.py scripts/check_site_seo.py
