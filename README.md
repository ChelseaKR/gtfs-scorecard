...
| Release & Versioning | Applies — reusable Action tags (`v1`; the newest version tag is `v1.4.0`), monthly dataset releases; the MCP server manifest is written but not published to the registry (see Versioning) | [`RELEASE-AND-VERSIONING-STANDARD.md`](docs/standards/RELEASE-AND-VERSIONING-STANDARD.md) |
