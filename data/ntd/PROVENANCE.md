...
Both are regenerated by `pipeline/scripts/ntd_reporter_coverage.py`.
`docs/findings/2026-08-15-ntd-reporters-without-a-discoverable-feed.md` is the
write-up.

**Read by the site since 2026-08-27.** `ntd_coverage.published_reporter_coverage`
reads the JSON and the renderer publishes its counts on `/ntd/` and in
`ntd.json` under `reporter_coverage` (ADR 0052). The CSV remains analysis
output and is not read by anything.

The JSON is a published input now, so it fails closed. A snapshot that does not
declare `unit: ntd_reporters`, or whose tier counts do not sum to its own
`obligated_reporters`, publishes nothing at all rather than a number a reader
would take for the real denominator. Regenerating it without re-running
`pipeline/tests/test_ntd_reporter_publication.py` is how that would be
noticed.
